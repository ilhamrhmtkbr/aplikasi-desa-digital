  ...
  actions: {
    ...
    async create(formData) {
      ...
      const fd = buildFormData(formData);
        const response = await axiosInstance.post('/family-member', fd, {
    ...
    async edit(formData, id) {
      ...
      try {
        ...
        const response = await axiosInstance.post(`/family-member/${id}`, fd, {
                            headers: { 'Content-Type': 'multipart/form-data' }
                        });
        this.success = response.data.message;
        router.push({name: 'familyMemberManage', params: {id: id}})
      } catch (error) {
        this.error = handleError(error);
      } finally {
        this.loading = false;
